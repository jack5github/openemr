...
{
    return sqlStatement(
        "SELECT d.id, d.name, a.line1, a.city, " .
...
function generate_select_list(
    ...
    for ($active = 1; $active == 1 || ($active == 0 && $include_inactive); --$active) {
        ...
        while ($lrow = sqlFetchArray($lres)) {
            ...
            if ((strlen($currvalue ?? '') == 0 && $lrow['is_default'] && !$ignore_default) 
...
function generate_form_field($frow, $currvalue)
{
    ...
    if ($data_type == 1 || $data_type == 43 || $data_type == 46) {
        ...
        while ($prow = sqlFetchArray($pres)) {
            ...
            $optionLabel = htmlspecialchars(
                $prow['name'] . ' ' . $prow['area_code'] . '-' .
        ...
        $ures = sqlStatement(
            "SELECT id, fname, lname, organization, username, npi FROM users " .
            "WHERE active = 1 AND ( info IS NULL OR info NOT LIKE '%Inactive%' ) " .
            "AND $tmp " .
...
function generate_print_field($frow, $currvalue, $value_allowed = true)
{
    ...
    if ($data_type == 1 || $data_type == 26 || $data_type == 33 || $data_type == 43 || $data_type == 46) {
        ...
        if ($currvalue) {
            $urow = sqlQuery(
                "SELECT fname, lname, specialty FROM users " .
...
function generate_display_field($frow, $currvalue, $as_csv = false)
{
    ...
    if ($data_type == 1 || $data_type == 26 || $data_type == 43 || $data_type == 46) {
        ...
        $urow = sqlQuery(
            "SELECT fname, lname, specialty, organization FROM users " .
            "WHERE id = ?", array($currvalue)
        );
...
function getLayoutProperties($formtype, &$grparr, $sel = "grp_title", $limit = null)
{
    ...
    $gres = sqlStatement(
        "SELECT $sel FROM layout_group_properties WHERE grp_form_id = ? " .
...
function display_layout_tabs($formtype, $result1, $result2 = '')
{
    ...
    while ($prow = sqlFetchArray($pres)) {
        ...
        $fres = sqlStatement(
            "SELECT distinct group_id FROM layout_options " .
            "WHERE form_id = ? AND uor > 0 " .
...
function get_layout_form_value($frow, $prefix = 'form_')
{
    ...
    if ($maxlength && $maxlength != 0 && mb_strlen(trim($value)) > $maxlength && !$frow['list_id']) {
        die(
            htmlspecialchars(xl('ERROR: Field') . " '$field_id' " . xl('is too long'), ENT_NOQUOTES) .
 ...
 *                                   use '0' for "unspecified facility" use ''
 *                                   for "All facilities" (the default)for "All facilities" (the default)
 *
...
function lbf_current_value($frow, $formid, $encounter)
{
    ...
    if ($source == 'D' || $source == 'H') {
        ...
        $ldrow = sqlQuery(
            "SELECT field_value FROM lbf_data WHERE " .
